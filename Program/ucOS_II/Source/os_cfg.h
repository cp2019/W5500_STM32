...
#define OS_ARG_CHK_EN             0    	//允许 (1) 或者禁止 (0) 变量检查 
#define OS_CPU_HOOKS_EN           1		// 为1时，hooks函数只能在os_cpu.c中定义，为0时，可在任意文件中定义 

#define OS_DEBUG_EN               0    	//使能DEBUG                                 */
...
#define OS_LOWEST_PRIO           25    /* 定义可分配的最低优先级Defines the lowest priority that can be assigned ...         */
                                       /* ... MUST NEVER be higher than 254!                           */

#define OS_MAX_EVENTS            8    //应用中最多事件控制块的数目 必须大于0 
#define OS_MAX_FLAGS             8    //应用中最多事件标志组的数目	必须大于 0	
#define OS_MAX_MEM_PART          5    //最多内存块的数目           MUST be > 0 
#define OS_MAX_QS                4    //应用中最多对列控制块的数目	MUST be > 0	
#define OS_MAX_TASKS             24   //应用中最多任务数目			MUST be >= 2 

#define OS_SCHED_LOCK_EN          1   //使能OSSchedLock() and OSSchedUnlock()
...
#define OS_TICKS_PER_SEC        1000    /* Set the number of ticks in one second                        */
                                       ...
                                       /* --------------------- TASK STACK SIZE ---------------------- */
#define OS_TASK_TMR_STK_SIZE    32    /*定时器任务堆栈大小 Timer      task stack size (# of OS_STK wide entries)        */
#define OS_TASK_STAT_STK_SIZE   32    //空闲任务堆栈容量 (#按照OS_STK的宽度数目) 
#define OS_TASK_IDLE_STK_SIZE   32    //统计任务堆栈容量( # 按照OS_STK的宽度数目)


                                       /* --------------------- TASK MANAGEMENT ---------------------- */
#define OS_TASK_CHANGE_PRIO_EN    0    //允许生成OSTaskChangePrio() 
#define OS_TASK_CREATE_EN         1    /*     Include code for OSTaskCreate()                          */
#define OS_TASK_CREATE_EXT_EN     0    /*     Include code for OSTaskCreateExt()  任务退出                     */
...
#define OS_TASK_STAT_STK_CHK_EN   0    /*     Check task stacks from statistic task 堆栈检查            */
...
#define OS_TMR_CFG_WHEEL_SIZE    8    /* 计数器大小    Size of timer wheel (#Spokes)                            */
#define OS_TMR_CFG_TICKS_PER_SEC 10    /* 定时器管理任务运行的速率（赫兹）    Rate at which timer management task runs (Hz)            */
